        ...
        <div className="grid lg:grid-cols-12 gap-20">
          ...
          <div className="lg:col-span-8 space-y-12">
            {cartItems.map((item: CartItem) => (
              <div key={`${item.product.id}-${item.size || 'no-size'}`} className="group relative grid grid-cols-1 sm:grid-cols-3 gap-8 pb-12 border-b border-neutral-100 last:border-0 animate-in fade-in slide-in-from-bottom-8 duration-700">
                
                {/* Image Section */}
                <Link to={`/product/${item.product.id}`} className="relative aspect-[3/4] overflow-hidden bg-neutral-50 block">
                   <img 
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-32 h-fit">
            <div className="bg-neutral-50/50 p-10 rounded-[2rem] border border-neutral-100 animate-in fade-in slide-in-from-right-8 duration-700 delay-200">
              ...
              <Link to="/checkout" className="block">
                <button className="w-full h-16 bg-gradient-to-tr from-neutral-900 via-neutral-800 to-neutral-700 text-white uppercase tracking-[0.6em] text-[11px] shadow-2xl hover:bg-neutral-800 transition-all flex items-center justify-center gap-4 group active:scale-[0.98] border-none">
                  Confirm Payment
